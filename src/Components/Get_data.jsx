...
export default function Get_data() {
    ...
    const feltered = value.filter(otdel => {
        const checksot = []
        otdel.ruc.sot.map(sot => {
            sot.toLowerCase().includes(invalue.toLowerCase())? checksot.push(true): checksot.push(false)
        })
        if (otdel.name.toLowerCase().includes(invalue.toLowerCase()) || otdel.ruc.name.toLowerCase().includes(invalue.toLowerCase()) || checksot.includes(true)) return true
    })
